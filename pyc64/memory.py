...
class Memory:
    ...
    def __init__(self, size=0x10000, endian="little"):
        ...
        self.hooked_reads = bytearray(
            size
        )  # 'bitmap' of addresses that have read-hooks, for fast checking
        self.hooked_writes = bytearray(
            size
        )  # 'bitmap' of addresses that have write-hooks, for fast checking
        self.rom_areas = (
            set()
        )  # set of tuples (start, end) addresses of ROM (read-only) areas
        self.endian = (
            endian  # 'little' or 'big', affects the way 16-bit words are read/written
        )
...
class ScreenAndMemory:
    ...
    def __init__(self, columns=40, rows=25, sprites=8, rom_directory=""):
        ...
        if rom_directory:
            ...
            self.memory._patch(
                0xE38A, 0xE3
            )  # ...(this jmp loop is recognised by tahe cpu emulator as an 'end of the program')
            self.memory._patch(
                0xFCF6, 0x90
            )  # skip a large part of the memory init routine that is very slow and may cause issues
        self.hz = 60  # NTSC
