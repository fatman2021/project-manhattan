...
sub MAIN(p as any ptr) 
	...
	MainWin = WindowCreate(19*32,13*32,@"SAME")
    drawable = GDICreate(MainWin,0,0,19*32,13*32)
    ...
    for x as unsigned integer= 0 to largeur-1
        for y as unsigned integer =0 to hauteur -1
            ...
            if (value<6) and (value>0) then
                GDIFillRectangle(drawable,x*32,y*32,x*32+31,y*32+31,couleurs(value))
            end if
            GDIDrawRectangle(drawable,x*32,y*32,x*32+31,y*32+31,&hFFFFFFFF)
        next y
    next x
end sub
