...
sub MessageBox.Show(message as unsigned byte ptr,title as unsigned byte ptr,btn as DialogButton,th as any ptr)
    
    newobj(msg,MessageBox)
    dim result as integer
    var ml = strlen(message)+1
    msg->Message = KAlloc(ml)
...
sub MessageBox_YesClicked(elem as TButton ptr)
    ...
    if (dlg->OwnerThread<>0) then
        var th =cptr(Thread ptr,dlg->OwnerThread)
        if (th->State=ThreadState.WaitingDialog) then
            th->HasModalVisible=0
            dim st as IRQ_Stack ptr = cptr(IRQ_Stack ptr,th->SavedESP)
            st->EAX = dlg->DialogResult
