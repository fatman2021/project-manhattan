Const hasfpu=1 ' always 1 if it has FPU (486DX)
const IDE_GRABA=1 ' for tests=1 record, DANGER!!!!!!

static shared As Integer CPUID ' cpu type, in intels before 486DX2-66, it is always 0. It only changes in the DX2-75
static shared As Integer cpu_multi ' CPU multiplier for DX2 type BUS (486dx2-66)
static shared As Integer CPUCLOCK ' ACTUAL speed 66Mhz
static shared As Integer cpu_exec ' for the EXEC386 () routine, it would be REALSPEED / 100
...
static shared As ULong oxpc ' copy of "pc" for some INS
...
static shared As UByte isram(0 To 255) ' I think it's okay with 256, because it's 256/16=16megs --> this is superfluous &h10000).
...
' variable used in register table "mod1add"
static shared As Integer slowrm(0 To 7)
' zero variable for register table  "mod1add"
...
' 32-bit structure
' if we do EAX=&h12345678 it outputs AL=78, AH=56, and EAX=5678
' if we do AL=00, EAX=12345600, AH=56, EAX=5600, etc.
Type x86reg
 Union
...
' Segments
' _cs,_ds,_es,_ss are segment structures
' CS,DS,ES,SS is the 16-bit data
'cs,ds,es,ss are defined to the bases
' (if I'm not mistaken, they are the "real" ones, for example: cs0 is CS1*16 (FFFF*16=FFFF0)
#Define CS1 _cs.seg
#Define DS1 _ds.seg
#Define ES1 _es.seg
#Define SS1 _ss.seg
...
' PIC i8259 module (IRQ interrupt control)
