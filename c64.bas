...
const as integer mZoom = 3 ' 1,2,3,4

type MEMORY_T
  public:
  declare constructor
  declare destructor
  declare function ReadByte   (byval adr as ushort) as  byte
  declare function ReadUByte  (byval adr as ushort) as ubyte
  declare function ReadUShort (byval adr as ushort) as ushort
  declare sub      WriteByte  (byval adr as ushort, byval b8  as   byte)
  declare sub      WriteUByte (byval adr as ushort, byval b8  as  ubyte)
  declare sub      WriteUShort(byval adr as ushort, byval w16 as ushort)

  private:
  declare function Peek8(byval adr as integer) as ubyte
  declare sub      Poke8(byval adr as integer, byval v as ubyte)
#if 0
  ' C64 memory map
  ...
  const as integer cia1_base  = &HDC00 ' 256 b  |      +- 4K I/O
...
#endif
  as ubyte   mem64 (&HFFFF) ' Ram
  as ubyte   kernal(&H1FFF) ' OS
  as ubyte   basic (&H1FFF) ' Basic
  as ubyte   char  (&H07FF) ' Font
  as ubyte   col   (&H03FF) ' color triples
end type

enum ADR_MODES
  _UNK ' unknow
  
  _IMP ' instruction only                  : implicit
  
  _IMM ' 1 byte operand                    : immidate
  ' opr = mem(pc)
  
  _ABS ' 2 byte lo hi                      : absolute
  ' adr = mem(pc) + mem(pc+1)*256
  
  _ZERO  ' 1 byte lo  (hi=0)               : zero page
  ' adr = mem(pc) and 255
  
  _ZEROX ' 1 byte lo  (hi=0)               : zero page X register
  ' adr = (mem(pc)+x) and 255
  
  _ZEROY ' 1 byte lo  (hi=09               : zero page Y register
  ' adr = (mem(pc)+y) and 255
  
  _ABSX  ' 2 byte lo hi                    : absolute X register
  ' adr = mem(pc ) + mem(pc+1)*256 + x
  
  _ABSY  ' 2 byte lo hi (abs y)            : absolute Y register
  ' adr = mem(pc ) + mem(pc+1)*256 + y
  
  _REL   ' 1 byte lo (branch -128 - +127)  : relative
  ' adr= PC + lo
  
  _INDX  ' 1 byte lo (ind x)               : indirect X register
  ' adr =(mem(pc )+x) and 255
  ' adr = mem(adr) + mem(adr+1)*256
  
  _INDY  ' 2 byte lo hi (ind y)            : indirect Y register
  ' adr = mem(pc ) + mem(pc +1)*256 + y
  
  _IND   ' 2 byte lo hi                    : indirect jump
  ' adr = mem(pc ) + mem(pc +1)*256
...
type CPU6510
  ...
  union ' stack pointer
    ...
    type
      ...
      as ubyte MSB   ' msb allways hi %FF
    end type
  end union
  as MEMORY_T ptr mem
  as OPCODE       code
...
constructor C64_T
  dprint("C64_T()")
  const as integer cSize=8*mZoom
  screenres 40*cSize +cSize*8,25*cSize+cSize*8,8
  for i as integer = 0 to 15
    dim as integer c=any
    read c:palette i,c
  next
  line (0,0)-(40*cSize+cSize*cSize-1,25*cSize+cSize*cSize-1),3,bf
  mem = new MEMORY_T()
  cpu = new CPU6510(mem)
end constructor
...
function MEMORY_T.peek8(byval adr as integer) as ubyte
  select case adr
  case &HE000 to &HFFFF
    return kernal(adr-&HE000)
  case &HA000 to &HBFFF
    return basic (adr-&HA000)
  case &HD800 to &HDBFF
    return col   (adr-&HD800)
  case &HD000 to &HD3FF
...
sub MEMORY_T.poke8(byval adr as integer,byval v as ubyte)
  const as integer cSize=8*mZoom
  mem64(adr)=v
  ...
  select case adr
    case 1024 to 2023
      ...
      dim as integer xs=adr mod 40:xs*=cSize:xs+=cSize*4
      dim as integer ys=adr  \  40:ys*=cSize:ys+=cSize*4
      screenlock
      for y as integer = 0 to 7
        for x as integer = 0 to 7
          if char(c) and (128 shr x) then
            line (xs+x*mZoom,ys+y*mZoom)-step(mZoom-1,mZoom-1),3,BF
          else
            line (xs+x*mZoom,ys+y*mZoom)-step(mZoom-1,mZoom-1),col(adr),BF
          end if
        next
        c+=1
      next
      screenunlock ys,ys+cSize
  end select
end sub

function MEMORY_T.ReadUByte(byval adr as ushort) as ubyte
  return peek8(adr)
end function

function MEMORY_T.ReadByte(byval adr as ushort) as byte
  return peek8(adr)
end function

function MEMORY_T.ReadUShort(byval adr as ushort) as ushort
  return peek8(adr) or peek8(adr+1) shl 8
end function

sub MEMORY_T.WriteByte(byval adr as ushort,byval b8 as byte)
  poke8(adr,b8)
end sub

sub MEMORY_T.WriteUByte(byval adr as ushort,byval b8 as ubyte)
  poke8(adr,b8)
end sub

sub MEMORY_T.WriteUShort(byval adr as ushort,byval w16 as ushort)
...
constructor CPU6510(byval lpMem as MEMORY_T ptr)
  ...
  'opcode,name,adrmode,bytes,ticks,operand,decoder
...
#else
 function CPU6510.Tick as integer
#endif   
...
' 6510 instructions
'
' unknow or undefuined instruction
...
sub INS_BIT(byval Cpu as CPU6510_T)
  ...
  Cpu->F.z=iif(0=(b and Cpu->sA),1,0)
end sub
...
sub INS_SBC(byval Cpu as CPU6510_T)
  ...
  Cpu->F.v=iif((((Cpu->A xor b.ulo) and &H80) and ((Cpu->A xor v.ulo) and &H80)),1,0)
  Cpu->A=v.ulo
  Cpu->F.c=iif(v.u16<=255,1,0)
  Cpu->F.z=iif(v.ulo =  0,1,0)
  Cpu->F.n=iif(v.slo <  0,1,0)
...
#ifdef DEBUG  
  ' in debug mode register the return key
  if key=13 then
    flag=1
  else
    flag=0
