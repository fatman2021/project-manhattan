...
static shared as ulong fc, bc, x0, x1, y0, y1, prompt, mx, my, mb
static shared as string cmd, getstr, msg
...
type MEMORY_T
  ...
  declare function Peek8(byval adr as ulongint) as ulongint
  declare sub      Poke8(byval adr as ulongint, byval v as ulongint)
#if 0
...
#endif
  as ulongint   mem64 (&HFFFE) ' Ram
  as ulongint   kernal(&H1FFF) ' OS
  as ulongint   basic (&H1FFF) ' Basic
  as ulongint   char  (&H07FF) ' Font
  as ulongint   col   (&H03E7) ' color triples
end type
...
function MEMORY_T.peek8(byval adr as ulongint) as ulongint
  select case adr
...
sub MEMORY_T.poke8(byval adr as ulongint,byval v as ulongint)
  mem64(adr)=v
  ...
  select case adr
    ...
    case sys_offset+2 ' Foreground Red
     fr = v: fc = rgba(fr,fg,fb,fa)
    case sys_offset+3 ' Foreground Green
     fg = v: fc = rgba(fr,fg,fb,fa)
    case sys_offset+4 ' Foreground Blue
     fb = v: fc = rgba(fr,fg,fb,fa)
    case sys_offset+5 ' Foreground Alpha
     fa = v: fc = rgba(fr,fg,fb,fa)
    case sys_offset+6 ' Background Red
     br = v: bc = rgba(br,bg,bb,ba)
    case sys_offset+7 ' Background Green
     bg = v: bc = rgba(br,bg,bb,ba)
    case sys_offset+8 ' Background Blue
     bb = v: bc = rgba(br,bg,bb,ba)
    case sys_offset+9 ' Background Alapha
     ba = v: bc = rgba(br,bg,bb,ba)
    case sys_offset+10
     x0 = mem64(sys_offset+11) shl 32 + mem64(sys_offset+12) shl 24 +_
		  mem64(sys_offset+13) shl 16 + mem64(sys_offset+14) shl 08 +_
		  mem64(sys_offset+15)
	case sys_offset+16
	 y0 = mem64(sys_offset+17) shl 32 + mem64(sys_offset+18) shl 24 +_
		  mem64(sys_offset+19) shl 16 + mem64(sys_offset+20) shl 08 +_
		  mem64(sys_offset+21)
	case sys_offset+22
     x1 = mem64(sys_offset+23) shl 32 + mem64(sys_offset+24) shl 24 +_
		  mem64(sys_offset+25) shl 16 + mem64(sys_offset+26) shl 08 +_
		  mem64(sys_offset+27)
    case sys_offset+28
     y1 = mem64(sys_offset+29) shl 32 + mem64(sys_offset+30) shl 24 +_
	...
	case sys_offset+36
	  ...
	  if mb and 1 then poke8(sys_offset+37, 1) ' left
	  if mb and 2 then poke8(sys_offset+37, 2) ' right
	  if mb and 3 then poke8(sys_offset+37, 3) ' middle
	  WriteUShort(sys_offset+38, mx): WriteUShort(sys_offset+39, my)
    case sys_offset+40 ' clear the screen
     cls
    case sys_offset+41 ' opens POV-Ray device
     open "tmp.pov" for output as #1
     print #1, "start"
    case sys_offset+42 ' closes device
     print #1, "end"
     close #1
    case sys_offset+43 ' Include header
	 select case v
    ...
    case sys_offset+44 ' graphics functions
     select case v
       case 0 : print #1, "camera {"
       case 1 : print #1, "light_source {"
       case 2 : print #1, "plane {"
       case 4 : print #1, "sphere {"
     end select
    case sys_offset+45 ' graphics sub functions
     select case v
      case 0 : print #1, "location ";
      case 1 : print #1, "look_at ";
      case 2 : print #1, "color ";
      case 3 : print #1, "color rgb ";
      case 4 : print #1, "color rgbf ";
      case 5 : print #1, "x, " + str(mem64(sys_offset+46))
      case 6 : print #1, "y, " + str(mem64(sys_offset+46))
      case 7 : print #1, "z, " + str(mem64(sys_offset+46))
      case 8 : print #1, "pigment {"
      case 9 : print #1, "checker"
      case 10: print #1, "scale ";
      case 11: print #1, "phong ";
      case 12: print #1, "reflection ";
      case 13: print #1, "finish {"
     end select 
    case sys_offset+47 ' vectors
     select case v
      case 0
       print #1, "< "+str(mem64(sys_offset+48));
       print #1, ", "+str(mem64(sys_offset+49));
       print #1, ", "+str(mem64(sys_offset+50))+" >"
      case 1
       print #1, "< "+str(mem64(sys_offset+48));
       print #1, ", "+str(mem64(sys_offset+49));
       print #1, ", "+str(mem64(sys_offset+50))+" >, ";
       print #1, str(mem64(sys_offset+51))
      case 2
       print #1, "< "+str(mem64(sys_offset+48));
       print #1, ", "+str(mem64(sys_offset+49));
       print #1, ", "+str(mem64(sys_offset+50))+">" 
       print #1, ", "+str(mem64(sys_offset+51))+"."+str(mem64(sys_offset+52)); 
      case 3
       print #1, "< "+str(mem64(sys_offset+48))+"."+str(mem64(sys_offset+49));
       print #1, ", "+str(mem64(sys_offset+50))+"."+str(mem64(sys_offset+51));
       print #1, ", "+str(mem64(sys_offset+52))+"."+str(mem64(sys_offset+53));
       print #1, " >"
      case 4
       print #1, "< "+str(mem64(sys_offset+48))+"."+str(mem64(sys_offset+49));
       print #1, ", "+str(mem64(sys_offset+50))+"."+str(mem64(sys_offset+51));
       print #1, ", "+str(mem64(sys_offset+52))+"."+str(mem64(sys_offset+53));
       print #1, ", "+str(mem64(sys_offset+54))+"."+str(mem64(sys_offset+55));
       print #1, " >"
      case 5
       print #1, str(mem64(sys_offset+48))
      case 6
    ...
    case sys_offset+56' return from function
     print #1, "}"
    case sys_offset+57 ' Render                    
      shell "povray -d -H600 -W800 tmp.pov"
	  shell "convert tmp.png tmp.bmp"
	  shell "mv tmp.bmp vram/"+str(mem64(sys_offset+59))+".bmp"
	  shell "rm tmp.pov tmp.png"
	  poke8(sys_offset+40,0) ' clear screen
	  bload "vram/"+str(mem64(sys_offset+59))+".bmp",image
	  put(0,0), image, alpha	       
	  'locate 1, 1: print "Press any key to continue."
	  'sleep
	  'poke8(11,0) ' clears the screen
    case sys_offset+58 ' Sets offset in video memory
      poke8(mem64(sys_offset+59),v)
    case sys_offset+60 ' Anamation player
	  poke8(sys_offset+40,0) ' clear screen
      for frame as ubyte = mem64(sys_offset+61) to mem64(sys_offset+62)
 	  bload "vram/"+str(frame)+".bmp", image
 	  sleep(10,1): put(0,0), image, pset   
      next frame
    case sys_offset+63 ' sets system_offset
      sys_offset = mem64(sys_offset+64) shl 16 + mem64(sys_offset+65) shl 08 +_
		           mem64(sys_offset+66)
    case sys_offset+67 ' resets screen  	    
	 cls : bload "background.bmp",0
     for offset = 0 to 1024: poke8(1024+offset, 32): next offset	  
    case 1024 to 2023
      adr-=1024
      ...
      for y as integer = 0 to 7
        for x as integer = 0 to 7
          if char(c) and (128 shr x) then
            pset(((xs+x)*2)-2,(ys+y)*2),fc: pset(((xs+x)*2),((ys+y)*2)-2),fc: pset((((xs+x)*2)-2),((ys+y)*2)-2),fc
            pset(((xs+x)*2)-1,(ys+y)*2),fc: pset(((xs+x)*2),((ys+y)*2)-1),fc: pset((((xs+x)*2)-1),((ys+y)*2)-1),fc           
            pset(((xs+x)*2),((ys+y)*2)),fc
          else
            pset(((xs+x)*2)-2,(ys+y)*2),bc: pset(((xs+x)*2),((ys+y)*2)-2),bc: pset((((xs+x)*2)-2),((ys+y)*2)-2),bc
            pset(((xs+x)*2)-1,(ys+y)*2),bc: pset(((xs+x)*2),((ys+y)*2)-1),bc: pset((((xs+x)*2)-1),((ys+y)*2)-1),bc                      
            pset(((xs+x)*2),((ys+y)*2)),bc
          end if
        next 
        c+=1
      next
...
sub INS_R32(byval Cpu as CPU6510_T)
  dim as multi v
  Cpu->A=Cpu->mem->peek8(Cpu->Code.op.u32) 
end sub

sub INS_W32(byval Cpu as CPU6510_T)
  Cpu->mem->Poke8(Cpu->code.op.u32,Cpu->A)
end sub

sub INS_R64(byval Cpu as CPU6510_T)  
  dim as multi v
  Cpu->A=Cpu->mem->peek8(Cpu->Code.op.u64)     
end sub

sub INS_W64(byval Cpu as CPU6510_T)
  Cpu->mem->Poke8(Cpu->code.op.u64,Cpu->A)
end sub
